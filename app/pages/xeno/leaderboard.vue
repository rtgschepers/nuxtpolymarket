...
const me = computed(() => players.value?.find(p => p.isCurrentUser))
const myRank = computed(() => {
  const i = players.value?.findIndex(p => p.isCurrentUser) ?? -1
  return i >= 0 ? i + 1 : null
})
</script>
...
<template>
  <UContainer class="pt-6">
    <div class="xeno-panel xeno-panel-accent rounded-3xl p-5 mb-6 flex flex-wrap items-center gap-5">
      <div class="xeno-orb size-16 shrink-0">
        ...
        <template #progress>
          <div class="hidden sm:block flex-1 mr-3 min-w-0 max-w-48">
            ...
            <div class="h-1.5 rounded-full bg-black/20 overflow-hidden">
              <div class="h-full rounded-full bg-primary shadow-[0_0_6px_var(--ui-primary)]" :style="{ width: `${Math.round(p.speciesUnlocked / totalSpecies * 100)}%` }" />
            </div>
          </div>
        </template>
        <template #stats>
          <div class="hidden md:flex items-center gap-4 shrink-0">
            <div class="flex flex-col items-center gap-0.5"><span class="text-sm font-bold">{{ formatNumber(p.plantCount, false, 0) }}</span><span class="text-[10px] text-muted">Plants</span></div>
            <div class="flex flex-col items-center gap-0.5"><span class="text-sm font-bold">{{ p.gridSlots }}</span><span class="text-[10px] text-muted">Plots</span></div>
            <div class="flex flex-col items-center gap-0.5"><span class="text-sm font-bold">{{ p.breederSlots }}</span><span class="text-[10px] text-muted">Pods</span></div>
            <div class="flex flex-col items-center gap-0.5"><span class="text-sm font-bold">{{ p.artifactCount }}</span><span class="text-[10px] text-muted">Relics</span></div>
          </div>
        </template>
        <template #trailing>
          ...
          <template #middle>
            <div class="hidden sm:flex flex-1 items-center gap-2 min-w-0">
              ...
              <div class="flex-1 h-1 rounded-full bg-black/20 overflow-hidden max-w-32">
                <div class="h-full rounded-full bg-primary/60" :style="{ width: `${Math.round(p.speciesUnlocked / totalSpecies * 100)}%` }" />
              </div>
            </div>
          </template>
          <template #stats>
            <div class="hidden md:flex items-center gap-3 shrink-0 text-xs">
              ...
              <span class="text-muted">{{ p.gridSlots }} <span class="text-muted/60">plots</span></span>
              <span class="text-muted">{{ p.breederSlots }} <span class="text-muted/60">pods</span></span>
              <span class="text-muted">{{ p.artifactCount }} <span class="text-muted/60">relics</span></span>
            </div>
          </template>
          <template #trailing>
            <div class="shrink-0 ml-auto"><CoinBalance :value="p.portfolioValue" /></div>
          </template>
        </LeaderboardListRow>
      </div>
    </div>

    <UEmpty v-else description="No players have started Xeno yet" icon="i-lucide-sprout" />
  </UContainer>
</template>
