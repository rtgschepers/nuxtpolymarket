// Xeno Slot sound playback. Every effect is synthesized on the fly
// (app/utils/slots/xenoslot-synth.ts); nothing is fetched. Same shape as
...
// speakers. The reel whir feeds the same master gain; the ambient music has
// its own gain into the compressor, so effects and music have separate volumes.

import {
    XENO_SOUND_COOLDOWNS,
    XENO_SOUND_DEFAULT_VOICE_CAP,
    XENO_SOUND_LEVELS,
    XENO_SOUND_MAX_VOICES,
    XENO_SOUND_VOICE_CAPS,
    XENO_SYNTH_DEFAULT_JITTER,
    XENO_SYNTH_JITTER,
    XENO_SYNTH_RECIPES,
    XenoAmbient,
    XenoReelWhir,
    ...
    xenoSynthWarmUp,
    type XenoSoundEvent
} from '~/utils/slots/xenoslot-synth'

export type { XenoSoundEvent }

const STORAGE_ENABLED = 'xenoslot-sound-enabled'
const STORAGE_VOLUME = 'xenoslot-sound-volume'
const STORAGE_MUSIC = 'xenoslot-music-volume'
const STORAGE_MUSIC_LEGACY = 'xenoslot-music-enabled'
...
interface ActiveVoice {
    event: XenoSoundEvent
    voice: XenoSynthVoice
}

let ctx: AudioContext | null = null
let bus: MasterBus | null = null
const voices: ActiveVoice[] = []
const lastPlayedAt = new Map<XenoSoundEvent, number>()
let whir: XenoReelWhir | null = null
let music: XenoAmbient | null = null
...
function play(event: XenoSoundEvent, intensity = 0, delay = 0) {
    const context = runningContext()
    if (!context) return
    const now = performance.now()
    if (now - (lastPlayedAt.get(event) ?? -Infinity) < XENO_SOUND_COOLDOWNS[event]) return
    lastPlayedAt.set(event, now)

    const cap = XENO_SOUND_VOICE_CAPS[event] ?? XENO_SOUND_DEFAULT_VOICE_CAP
    let same = 0
    ...
    while (voices.length >= XENO_SOUND_MAX_VOICES) steal(voices[0]!)

    const jitter = XENO_SYNTH_JITTER[event] ?? XENO_SYNTH_DEFAULT_JITTER
    const voice = new XenoSynthVoice(context, ensureBus(context).input, {
        level: XENO_SOUND_LEVELS[event],
    ...
    voices.push(entry)
    try {
        XENO_SYNTH_RECIPES[event](voice, intensity)
    } catch {
        voice.release(0)
    }
    voice.seal()
...
function startWhir() {
    ...
    if (!context) return
    whir?.stop(0.05)
    whir = new XenoReelWhir(context, ensureBus(context).input, WHIR_LEVEL)
}
...
function startMusic() {
    ...
    xenoSynthWarmUp(context)
    music = new XenoAmbient(context, ensureBus(context).music, MUSIC_LEVEL)
...
function initialize() {
    ...
    } else if (readStorage(STORAGE_MUSIC_LEGACY) === 'false') {
        musicVolume.value = 0
    }
...
export function useXenoSlotSound() {
    initialize()
