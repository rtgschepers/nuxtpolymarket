...
const thornwick: WorldScene = {
    ...
    draw(s, sc, t) {
        ...
        // The framing pines: nearest the camera, so they travel at full scroll speed and sweep
        // across the fight mid-march. Laid out on SCROLL_PERIOD rather than scattered by hash,
        // so that every time the party stops they are back framing the two edges exactly as they
        // started — a trunk parked across the battle for a whole fight would be far worse than
        // holding still.
        const base = -mod(sc, SCROLL_PERIOD)
        for (let k = 0; k <= 1; k++) {
            const o = base + k * SCROLL_PERIOD
            bigPine(s, o + 14, FLOOR_Y, 86, C.moss0, C.moss2)
            bigPine(s, o + 34, FLOOR_Y, 58, C.moss0, C.moss2)
            bigPine(s, o + 280, FLOOR_Y, 62, C.moss0, C.moss2)
            bigPine(s, o + 296, FLOOR_Y, 92, C.moss0, C.moss2)
        }
