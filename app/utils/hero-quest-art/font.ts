// Two bitmap fonts: Pixel Crusade's 3×5 for small readouts, and a 5×7 for damage numbers,
// banners and the logo. Glyphs are row-major bit strings. Drawing never allocates — text is
// walked by char code.
...
interface Font { w: number, h: number, bits: Uint8Array[], widths: Uint8Array }

function build(src: Record<string, string>, w: number, h: number, trim: boolean): Font {
    ...
    return { w, h, bits, widths }
}

const FONTS = { small: build(SMALL, 3, 5, true), big: build(BIG, 5, 7, false) }
...
export function textWidth(text: string, font: FontName = 'small', scale = 1): number {
    const f = FONTS[font]
    let w = 0
    for (let i = 0; i < text.length; i++) w += (f.widths[norm(text.charCodeAt(i))]! + 1) * scale
