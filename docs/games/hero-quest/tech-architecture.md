...
**Automatic boss engagement (2026-08-18).** The client calls `boss/engage.post.ts` on its own while `document.visibilityState` reads `visible` (`useHqAutoBoss`, decision logic pure and specced in `app/utils/hero-quest-auto-boss.ts`). Nothing server-side changed for it: the route still settles, re-checks the position under the row lock and rejects anything not parked at a gate. What changed is that **the 400 "not at a boss gate" is now a routine response** — the client projects kills fractionally and the settle floors them, so it can arrive a beat early. The client waits `AUTO_ENGAGE_GRACE_KILLS` (one kill, clamped to 1–10s) to make that rare and retries after `AUTO_ENGAGE_RETRY_SECONDS` to make it harmless. **Do not soften that rejection** — it is the check that stops a client's optimism from moving the run. A hidden tab never engages, so bosses still require presence (`idle-mechanics.md` §2, §5).
