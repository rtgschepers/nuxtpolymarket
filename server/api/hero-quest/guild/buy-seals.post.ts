...
import { ladderDateKey, sealLadderPrice, sealLadderTotal } from '#shared/utils/hero-quest/gacha'

/** Bulk buys are priced rung by rung, so this bounds the loop and the Gold spent per call. */
const MAX_SEALS_PER_PURCHASE = 10

/**
 * Buy extra Guild Seals with Gold, on the daily escalating ladder (`gold-economy.md` §7).
 *
 ...
 * behind one shared reset date**, so buying Champion pulls today never moves the price of
 * Skill pulls.
 *
 * This is the designed Gold sink — the reason banked Gold is worth anything at all, since
 * Gold buys nothing else in the game. The ladder is what stops a whale converting an
 * arbitrarily large Gold pile into an arbitrarily large collection in one sitting.
 *
 * ## Concurrency — lock-then-read, not claim-then-reward
 *
 * The price *depends on the counter's old value*, and the counter lives inside a jsonb map,
 * so there is no single column to compare-and-swap and no flag to conditionally flip. That
 * makes this the textbook case for pattern B: take the `hqState` row lock, read the counter
 * **inside** the lock, price against it, and write everything in the same transaction. A
 * counter read before the lock is already stale, and two concurrent buys would both price at
 * the base rung.
 *
 * `debit` is threaded with `tx` — mandatory, not stylistic. Without it the Gold write goes out
 * on a second pool connection and deadlocks against the lock this transaction is holding.
 */
export default defineEventHandler(async (event) => {
    ...
    return db.transaction(async (tx) => {
        ...
        const total = sealLadderTotal('champion', boughtToday, requested)

        // Throws 400 on an insufficient balance — no manual check, and no window between
        // checking and spending.
        await debit(userId, total.toFixed(4), 'hero-quest:guild-seals', tx)

        const [updated] = await tx.update(hqState)
            .set({
                guildSeals: sql`${hqState.guildSeals} + ${requested}`,
                sealLadderPurchasedToday: { ...counters, champion: boughtToday + requested },
